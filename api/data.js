...
export default async function handler(req) {
  ...
  // 获取订单数据
  if (req.method === 'GET' && new URL(req.url).searchParams.get('type') === 'order') {
  ...
  // 保存订单数据
  if (req.method === 'POST' && new URL(req.url).searchParams.get('type') === 'order') {
  ...
  // 保存查询日志
  if (req.method === 'POST' && new URL(req.url).searchParams.get('type') === 'log') {
  ...
  // 获取查询日志
  if (req.method === 'GET' && new URL(req.url).searchParams.get('type') === 'log') {
